...
init: $(INIT_MARKER) docs ## [Core] Set up your environment
	...
	@export PATH="/home/$$USER/.cargo/bin:$$PATH" && \
		uv pip install -e .
	@mkdir -p data/{agents,memory,dialogue,reflection,summary,embeddings,spider,sanity}
	@echo "Checking API keys..."
	@for key in ANTHROPIC_API_KEY GEMINI_API_KEY OPENAI_API_KEY AWS_ACCESS_KEY_ID; do \
		if [ -z "$${!key}" ]; then \
			echo "⚠️  Warning: $$key not set"; \
		else \
			echo "✅ $$key is set"; \
		fi \
	done
...
check: ## [LLM] Verify LLM tools are working
	...
	@export PATH="/home/$$USER/.cargo/bin:$$PATH" && { \
		echo -n "LLM: "; $(LLM) --version 2>/dev/null || { echo "❌ not found"; exit 1; }; \
		echo -n "ttok: "; uv run ttok --version 2>/dev/null || { echo "❌ not found"; exit 1; }; \
		echo -n "strip-tags: "; uv run strip-tags --version 2>/dev/null || { echo "❌ not found"; exit 1; }; \
		echo -n "files-to-prompt: "; uv run files-to-prompt --version 2>/dev/null || { echo "❌ not found"; exit 1; }; \
	}
...
sanity: ## [Dev] Run sanity checks on core targets
	...
	@for log in data/sanity/*.log; do \
		echo "\nAnalyzing $${log}..."; \
		llm "Analyze this make target output log and suggest specific improvements for the Makefile target and any related files/configuration. Focus on: \
		1. Error handling \
		2. Dependencies \
		3. Performance \
		4. User feedback \
		5. Resource management \
		\nProvide specific, actionable recommendations." "$${log}" 2>&1 | tee "$${log}.analysis"; \
	done
	@echo "Sanity check complete. See data/sanity/*.log.analysis for recommendations"
...
embeddings: $(EMBEDDINGS_ORG) ## [Advanced] Try vector embeddings with images
	@$(TANGLE_SCRIPT) $(EMBEDDINGS_ORG) | tee $(DATA_DIR)/tangle-embeddings.log
...
sin: $(SIN_ANALYSIS_OUTPUT) ## [Advanced] Run Structured Insight Navigator (SIN) framework for systematic analysis
